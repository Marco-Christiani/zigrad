...
from dependency_metadata import XlaCudaMetadata, llvm_version
...
def requirement_for(
    ...
    if name == "xla-llvm":
        return Requirement(
            consumer=consumer,
            kind="exact-revision",
            revision=XlaCudaMetadata(xla_source).llvm_revision(),
    ...
    if name == "iree-llvm":
        return Requirement(
            consumer=consumer,
            kind="exact-revision",
            revision=snapshot["iree_llvm"]["rev"],
...
def select_candidate(
    ...
    source_name = "iree_llvm" if group == "iree-build-llvm" else "llvm"
...
def plan_configuration(
    ...
    entries = configuration.get("compatibility", [])
