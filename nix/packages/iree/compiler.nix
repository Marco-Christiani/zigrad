...
assert lib.assertMsg (targetBackends != []) "the IREE compiler requires a target backend";
assert lib.assertMsg (!lib.elem "cuda" targetBackends || cudaToolkit != null)
"the IREE CUDA compiler requires cudaToolkit";
  stdenv.mkDerivation {
    pname = "iree-compiler-${lib.concatStringsSep "-" targetBackends}";
    ...
    buildPhase = ''
      ...
      cmake -S iree-src -B iree-build -G Ninja \
        ...
        ${lib.concatMapStringsSep " \\\n+        " (backend: "-DIREE_TARGET_BACKEND_${lib.toUpper (lib.replaceStrings ["-"] ["_"] backend)}=ON") targetBackends} \
        \
    ...
    installPhase = ''
      ...
      # The compiler must load the matching library even when the caller's
      # environment contains another IREE installation.
      env -u LD_LIBRARY_PATH "$out/bin/iree-compile" --version
      ${lib.optionalString (lib.elem "llvm-cpu" targetBackends) ''
        env -u LD_LIBRARY_PATH "$out/bin/iree-compile" \
          --iree-llvmcpu-list-targets > registered-targets.txt
        grep -q '^    aarch64 ' registered-targets.txt
        grep -q '^    x86-64 ' registered-targets.txt
      ''}
