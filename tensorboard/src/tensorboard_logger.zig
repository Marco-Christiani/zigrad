...
pub const TensorBoardLogger = struct {
    ...
    pub fn init(log_dir: []const u8, allocator: std.mem.Allocator) !TensorBoardLogger {
    ...
    fn writeEvent(self: *TensorBoardLogger, event: *tb.Event) !void {
    ...
    pub fn addHistogram(self: *TensorBoardLogger, tag: []const u8, values: []const f64, step: i64) !void {
        var event = tb.Event.init(self.allocator);
        defer event.deinit();
        ...
        histo.min = findMin(values);
        histo.max = findMax(values);
        histo.num = @floatFromInt(values.len);
        histo.sum = sum(values);
        histo.sum_squares = sumSquares(values);
...
//
//     var histo = tb.HistogramProto{
//         .min = min,
//         .max = max,
//         .num = @floatFromInt(values.len),
//         .sum = blk: {
//             var total: f64 = 0;
...
//     try computeBuckets(&histo, values, num_buckets);
//
//     const histo_value = tb.Summary.Value.value_union{ .histo = histo };
//     try self.addSummary(tag, "histograms", histo_value, step);
// }
