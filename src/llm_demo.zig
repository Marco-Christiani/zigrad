...
pub fn run_llm_ft_demo(
    ...
    const tmp_b = try upload_host_buffer(allocator, &backend_handle, device, &host_b);
    ...
    if (!quiet) {
        std.log.info("llm-ft-demo donation: reused_params={d} replaced_params={d}", .{ reused_params, replaced_params });
    }
