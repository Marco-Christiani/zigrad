...
pub fn DQNAgent(comptime T: type, buffer_capacity: usize) type {
    return struct {
        ...
        pub fn selectAction(self: *Self, state: [4]T, step: usize, allocator: std.mem.Allocator) !u32 {
            const epsilon = self.eps_end + (self.eps_start - self.eps_end) * @exp(-1.0 * @as(T, @floatFromInt(step)) / self.eps_decay);
            // if (self.random.float(T) < epsilon) {
            if (std.crypto.random.float(T) <= epsilon) {
        ...
        pub fn train1(self: *Self, allocator: std.mem.Allocator) !T {
            ...
            const next_states = try NDTensor(T).init(next_states_flat, &[_]usize{ bs, 4 }, true, allocator);
            ...
            // std.debug.print("Sample Q-values: {d}\n", .{q_values.data.data[0..5]});
