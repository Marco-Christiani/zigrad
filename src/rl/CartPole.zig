...
masspole: T = 0.1,
total_mass: T,
length: T = 0.5, // half pole's length
polemass_length: T,
force_mag: T = 10.0,
tau: T = 0.02, // seconds between state updates
...
pub fn step(self: *Self, action: u32) struct { state: [4]T, reward: T, done: u1 } {
    const force = if (action == 1) self.force_mag else if (action == 0) -self.force_mag else @panic("Action value out of bounds");
    ...
    const done: u1 = @intFromBool(self.x < -self.x_threshold or self.x > self.x_threshold or self.theta < -self.theta_threshold_radians or self.theta > self.theta_threshold_radians);
    ...
    if (done == 0) { // not done
        ...
        if (self.steps_beyond_done.? == 0) {
            std.debug.print("Sim is over but you called reset. Further steps are undefined behavior.\n", .{});
        }
