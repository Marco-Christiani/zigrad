...
class Profiler:
    ...
    def log(self, loss, duration_ms):
        print(
            f"train_loss: {loss:.5f} [{self.batch}/{self.total_batches}] [ms/sample: {duration_ms:.4f}]"
        )

    def log_epoch(self, avg_loss, duration_ms):
        print(
            f"Epoch {self.epoch + 1}: Avg Loss = {avg_loss:.4f} ({duration_ms:.2f}ms)"
        )
...
def main(
    ...
    data_dir = Path(os.getenv("ZG_TEST_DATA_DIR", "/tmp"))
    ...
    model = (
        ModelAg(model_variant).to(device)
        if autograd
        else Model(model_variant).to(device)
    )
    ...
    for epoch in range(num_epochs):
        ...
        for i, (images, labels) in enumerate(dataloader):
            ...
            duration_ms = (
                (batch_end_time - batch_start_time) * 1000 / batch_size
            )  # ms per sample
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c", action="store_true", default=False, help="Whether to torch.compile()."
    )
    parser.add_argument(
        "-t", action="store_true", default=False, help="Whether to train the model."
    )
    parser.add_argument(
        "--grad_mode",
        type=GradMode,
        default="default",
        help="Set the PyTorch grad tracking policy.",
    )
    parser.add_argument(
        "--device",
        type=str,
