...
extern "C" CutensorPlanWrapper get_contraction_plan(
  ...
  // note: pass true to enable reversal for col-major
  BoundedArray<i64> a_dims(x_dims, x_dims_len);
  BoundedArray<i32> a_syms(x_syms, x_dims_len);

  BoundedArray<i64> b_dims(y_dims, y_dims_len);
  BoundedArray<i32> b_syms(y_syms, y_dims_len);

  BoundedArray<i64> c_dims(z_dims, z_dims_len);
  BoundedArray<i32> c_syms(z_syms, z_dims_len);

  cutensorTensorDescriptor_t x_desc;
  CUTENSOR_ASSERT(cutensorCreateTensorDescriptor(
    ct->handle, &x_desc, a_dims.size, a_dims.data, NULL, /*stride*/
    data_type, cutensor_alignment
  ));

  cutensorTensorDescriptor_t y_desc;
  CUTENSOR_ASSERT(cutensorCreateTensorDescriptor(
    ct->handle, &y_desc, b_dims.size, b_dims.data, NULL, /*stride*/
    data_type, cutensor_alignment
  ));

  cutensorTensorDescriptor_t z_desc;
  CUTENSOR_ASSERT(cutensorCreateTensorDescriptor(
    ct->handle, &z_desc, c_dims.size, c_dims.data, NULL, /*stride*/
    data_type, cutensor_alignment
  ));

  cutensorOperationDescriptor_t op_desc;
  CUTENSOR_ASSERT(cutensorCreateContraction(
    ct->handle, &op_desc, x_desc, a_syms.data, CUTENSOR_OP_IDENTITY, y_desc, b_syms.data,
    CUTENSOR_OP_IDENTITY, z_desc, c_syms.data, CUTENSOR_OP_IDENTITY, z_desc, c_syms.data,
  ...
  CUTENSOR_ASSERT(cutensorCreatePlanPreference(ct->handle, &plan_pref, algo, CUTENSOR_JIT_MODE_NONE)
  );

  len_t scratch_len;
  CUTENSOR_ASSERT(cutensorEstimateWorkspaceSize(
    ct->handle, op_desc, plan_pref, CUTENSOR_WORKSPACE_DEFAULT, &scratch_len
  ));

  cutensorPlan_t plan;
  CUTENSOR_ASSERT(cutensorCreatePlan(ct->handle, &plan, op_desc, plan_pref, scratch_len));
...
extern "C" void contraction(
  ...
  CUTENSOR_ASSERT(cutensorContract(
    backend->handle, reinterpret_cast<cutensorPlan_t>(plan), alpha, A_vals, x_vals, gamma, y_vals,
    y_vals, scratch, scratch_len, backend->stream
  ));
}
