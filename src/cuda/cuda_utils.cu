...
template <typename T>
__global__ void __mem_update_kernel(const T* src, T* dst, const len_t* idx, UPDATE_OP upd) {
  switch (upd) {
    case UPD_SET:
      dst[*idx] = *src;
      return;
...
// this is used for back propogation through min and max
extern "C" void memUpdateIndex(dtype id, void* dst, const void* src, const len_t* idx, UPDATE_OP upd, void* stream) {
  const auto _stream = static_cast<cudaStream_t>(stream);
  switch (id) {
    case SINGLE: {
        __mem_update_kernel<float><<<1,1,0,_stream>>>(
    ...
    case DOUBLE: {
        __mem_update_kernel<double><<<1,1,0,_stream>>>(
