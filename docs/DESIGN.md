...
The baseline realization path (no kernelization) must always exist and produce correct results. This is a correctness invariant. Whether the pipeline falls back to it silently is a policy decision governed by the two-tier model below.

**Tier 1 - Explicit kernelization (default).** The user explicitly requested kernelization (configured a provider, annotated regions, set `kernelize` options). Failure to produce, validate, or invoke a KA is a **hard error**. The user asked for specific behavior and must know when it cannot be fulfilled. Silent degradation to baseline is not acceptable here.

**Tier 2 - Opportunistic kernelization.** The system identifies kernelization candidates automatically, without user annotation (e.g., future auto-discovery or heuristic-driven selection). Failure to produce a KA falls back to baseline with a visible warning. The user did not request it, so degradation is acceptable - but must be observable.
